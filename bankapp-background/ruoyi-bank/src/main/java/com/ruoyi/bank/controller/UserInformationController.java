 ...
 */
@RestController
@RequestMapping("/bank/user_information")
public class UserInformationController extends BaseController
{
     ...
     */
    @PreAuthorize("@ss.hasPermi('bank:user_information:list')")
    @GetMapping("/list")
     ...
     */
    @PreAuthorize("@ss.hasPermi('bank:user_information:export')")
     ...
     */
    @PreAuthorize("@ss.hasPermi('bank:user_information:query')")
    @GetMapping(value = "/{useId}")
    public AjaxResult getInfo(@PathVariable("useId") String useId)
    {
        return success(userInformationService.selectUserInformationByUseId(useId));
    }

    /**
     * 新增User_information
     */
    @PreAuthorize("@ss.hasPermi('bank:user_information:add')")
     ...
     * 修改User_information
     */
    @PreAuthorize("@ss.hasPermi('bank:user_information:edit')")
     ...
     * 删除User_information
     */
    @PreAuthorize("@ss.hasPermi('bank:user_information:remove')")
    @Log(title = "User_information", businessType = BusinessType.DELETE)
	@DeleteMapping("/{useIds}")
    public AjaxResult remove(@PathVariable String[] useIds)
    {
        return toAjax(userInformationService.deleteUserInformationByUseIds(useIds));
    }
}
