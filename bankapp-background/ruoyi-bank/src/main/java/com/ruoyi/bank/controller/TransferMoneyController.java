 ...
 */
@RestController
@RequestMapping("/bank/transfer_money")
public class TransferMoneyController extends BaseController
{
     ...
     * 查询Transfer_money列表
     */
    @PreAuthorize("@ss.hasPermi('bank:transfer_money:list')")
    @GetMapping("/list")
     ...
     * 导出Transfer_money列表
     */
    @PreAuthorize("@ss.hasPermi('bank:transfer_money:export')")
     ...
     */
    @PreAuthorize("@ss.hasPermi('bank:transfer_money:query')")
    @GetMapping(value = "/{id}")
    public AjaxResult getInfo(@PathVariable("id") String id)
    {
        return success(transferMoneyService.selectTransferMoneyById(id));
    }

    /**
     * 新增Transfer_money
     */
    @PreAuthorize("@ss.hasPermi('bank:transfer_money:add')")
     ...
     * 修改Transfer_money
     */
    @PreAuthorize("@ss.hasPermi('bank:transfer_money:edit')")
     ...
     * 删除Transfer_money
     */
    @PreAuthorize("@ss.hasPermi('bank:transfer_money:remove')")
    @Log(title = "Transfer_money", businessType = BusinessType.DELETE)
	@DeleteMapping("/{ids}")
    public AjaxResult remove(@PathVariable String[] ids)
    {
        return toAjax(transferMoneyService.deleteTransferMoneyByIds(ids));
    }
}
